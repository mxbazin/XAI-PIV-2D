...
class uv_structure():
    ...
    def __init__(self,data_in={"uvw_folder":"../../P125_21pi_vu/","uvw_file":"P125_21pi_vu.$INDEX$.h5.uvw","Hperc":1.75,
        ...
        # ---------------------------------------------------------------------------------------------------------------
        # Store total area (kept under the legacy name voltot_plus for compatibility)
        # ---------------------------------------------------------------------------------------------------------------
        self.voltot_plus = flowfield.voltot
        self.mesh_vol    = flowfield.vol_h
    ...
    def calculate_matstruc(self):
        ...
        # ---------------------------------------------------------------------------------------------------------------
        # 2D PIV: shapes are (shpy, shpx). The RMS profile is (shpy,) and
        # broadcasts along x using reshape(-1, 1).
        # ---------------------------------------------------------------------------------------------------------------
        uv_xyz         = abs(np.multiply(self.field_u,self.field_v))
        uv_rms         = np.multiply(data_urms["uurms"],data_urms["vvrms"]).reshape(-1,1)
    ...
    def reconstruct_fromprevious(self, data_in=None):
        raise NotImplementedError("reconstruct_fromprevious is a 3D legacy helper and is not supported in 2D PIV.")
    ...
    def detect_quadrant(self):
        # 2D PIV: mesh_vol has shape (1, shpx); broadcast to (shpy, shpx) for masked sums.
        max_struc = int(np.max(self.structures.mat_segment))
        self.event = np.zeros(max_struc)
        area_map = np.broadcast_to(self.mesh_vol, (self.shpy, self.shpx))
        for index_struc in np.arange(max_struc):
            grid_struc  = self.structures.mat_segment == index_struc + 1
            struc_signu = np.sum(np.sign(self.field_u[grid_struc]) * area_map[grid_struc])
            struc_signv = np.sum(np.sign(self.field_v[grid_struc]) * area_map[grid_struc])
